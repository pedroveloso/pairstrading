...
pairs <- pairs %>% mutate(ITUB4 = log(Price1)) %>%
  mutate(BBDC4 = log(Price2)) %>%
...
unitRootP1 <- ur.ers(pairs_train$ITUB4,type="DF-GLS",model="const",lag.max=5)
summary(unitRootP1)
unitRootP2 <- ur.ers(pairs_train$BBDC4,type="DF-GLS",model="const",lag.max=5)
summary(unitRootP2)

# REGREDINDO UMA SERIE NA OUTRA PARA OBTER RESIDUO
m1 <- lm(ITUB4 ~ BBDC4, data = pairs_train)
wt <- m1$residuals
